...
filesystem_logs = [
    ...
    {"timestamp": random_timestamp().isoformat(), "log": "Created system backup"},
    {
        "timestamp": random_timestamp().isoformat(),
...
users_logs = [
    ...
    {"timestamp": random_timestamp().isoformat(), "log": "Added new user mohammed"},
    {"timestamp": random_timestamp().isoformat(), "log": "my password is 213214"},
    {
        "timestamp": random_timestamp().isoformat(),
...
packages_logs = [
    ...
    {"timestamp": random_timestamp().isoformat(), "log": "Installed nginx"},
    {
        "timestamp": random_timestamp().isoformat(),
    ...
    {"timestamp": random_timestamp().isoformat(), "log": "Configured python-venv"},
]
...
for domain, logs in domains_data.items():
    ...
    for log in logs[:2]:  # Display only top 2 logs
        time_str = datetime.fromisoformat(log["timestamp"]).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
...
def retrieve_domain_information(domain: str, query: str, max_results: int = 3) -> str:
    ...
    if (
        isinstance(max_results, dict)
        and "type" in max_results
        and max_results["type"] == "int"
    ):
...
def search_across_domains(
    ...
    if (
        isinstance(max_per_domain, dict)
        and "type" in max_per_domain
        and max_per_domain["type"] == "int"
    ):
    ...
    return rag_manager.multi_domain_retrieve_formatted(
        query, domain_list, max_per_domain
    )
...
class QueryTypeResult(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0, le=1, description="Confidence in classification"
    )
...
class LinuxAssistantFlow(Flow[LinuxAssistantState]):
    ...
    def domain_analysis(self, state: LinuxAssistantState):
        ...
        domain_response = domain_agent.execute_task(
            domain_task,
            context=json.dumps(
                {"prompt": self.state.prompt, "domains": ", ".join(self.state.domains)}
            ),
        )
    ...
    def router(self, state: LinuxAssistantState):
        ...
        if self.state.domain_analysis and self.state.current_domain_index < len(
            self.state.domain_analysis.domains
        ):
        ...
        # If query type determined but no response generated yet
            ...
            and self.state.query_type.query_type == "command"
            and not self.state.command_response
        ):
            return "generate_command_router"

        elif (
            self.state.query_type
            and self.state.query_type.query_type == "information"
            and not self.state.information_response
        ):
    ...
    def context_retrieval(self, state: LinuxAssistantState):
        ...
        current_domain = self.state.domain_analysis.domains[
            self.state.current_domain_index
        ]
    ...
    def query_classification(self, state: LinuxAssistantState):
        ...
        query_response = query_classifier_agent.execute_task(
            ...
            context=json.dumps(
                {"prompt": self.state.prompt, "context": combined_context}
            ),
        )
    ...
    def generate_command(self, state: LinuxAssistantState):
        ...
        if not any(
            phrase in self.state.command_response.explanation.lower()
    ...
    def generate_information(self, state: LinuxAssistantState):
        ...
        try:
            ...
            try:
                self.state.information_response = InformationResponse.parse_raw(
                    info_response
                )

                # Ensure the answer is personalized if not already
                if not any(
                    phrase in self.state.information_response.answer.lower()
                    for phrase in ["your", "you", "on your", "in your"]
                ):
                ...
                print(
                    f"Warning: Failed to parse information response as JSON: {str(e)}"
                )
