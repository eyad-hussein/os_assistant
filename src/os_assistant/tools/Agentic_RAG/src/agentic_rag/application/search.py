...
def search_logs(query: str, 
                ...
                auto_init: bool = True) -> Tuple[List[Dict[str, Any]], Optional[List[str]]]:
    """
    ...
    for i, chunk in enumerate(similar_chunks):
        domain_info = f"Domain: {chunk.get('domain', 'unknown')}, " if len(domains) > 1 else ""
    ...
    for i, log in enumerate(aggregated_logs):
        domain_info = f"Domain: {log.get('domain', 'unknown')}, " if len(domains) > 1 else ""
    ...
    if summarize and aggregated_logs:
        ...
        for i, summary in enumerate(summaries):
            domain_info = f"Domain: {aggregated_logs[i].get('domain', 'unknown')}, " if len(domains) > 1 else ""
