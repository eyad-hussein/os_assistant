...
def initialize_database(log_domain: LogDomain,
                        chunk_size: int = DEFAULT_CHUNK_SIZE, 
                        overlap: float = DEFAULT_CHUNK_OVERLAP,
                        batch_size: int = 5,
                        start_time: str = None,
                        end_time: str = None,
                        continue_from_last: bool = True) -> LogDatabase:
    """
    ...
    print(f"Database initialization complete in {elapsed_time:.2f} seconds. Processed {batch_count} batches.")
