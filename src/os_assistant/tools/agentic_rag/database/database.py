...
class LogDatabase:
    ...
    def get_domain(self) -> LogDomain | None:
    ...
    def is_initialized(self) -> bool:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT count(*) FROM sqlite_master WHERE type='table' AND name='log_chunks'"
        )
    ...
    def get_next_timestamp(self) -> str | None:
    ...
    def log_exists(self, log_number: int) -> bool:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT COUNT(*) FROM log_chunks WHERE log_number = ?", (log_number,)
        )
    ...
    def get_chunk_by_id(
        self, log_number: int, chunk_number: int
    ) -> dict[str, Any] | None:
