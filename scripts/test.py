...
def connect_to_db(db_path):
    ...
    if not os.path.exists(db_path):
        ...
        print(
            "Please run 'python main.py init --log-file users_logs.json' first to create the database."
        )
        exit(1)
...
def main():
    ...
    parser.add_argument(
        "--db-path", type=str, metavar="PATH", help="Path to SQLite database"
    )
    parser.add_argument(
        "--schema", action="store_true", help="Show table schema details"
    )
    parser.add_argument(
        "--log",
        type=int,
